...
typedef struct _st{
	MoteurStruct Moteur[2];
	int32_t TOROuput;
	int32_t SelectedMotor;
	int32_t ValidFlash;
...
#ifdef CPLUSPLUS
// fonction accesible depuis touchgx


 void SelectCurrentMoteur(int val);

 void StartMoteur(int moteur);
 void StopMoteur( int moteur);


 int MoteurView_GetADCInput(void);
 int MoteurView_GetSpeedMot(int moteurnum);

 ParaSauvegarde_t* Get_Parametre(void);
 int MoteurView_applique(MoteurStruct* mot , int moteurnum);
 int GetCurrentMoteur(void);

 void SetIOOutput(int bitmask);
 int GetIOInput(void);
 int GetADCInput(void);
 int restoreinitialIO(void);

 void AppliqueConsigneVitessePotar (int moteur);
 void ResetPos(int moteur);

 void setlimitMinMax (int moteur);
