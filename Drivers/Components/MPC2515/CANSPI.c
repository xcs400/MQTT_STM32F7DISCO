...
uint8_t CANSPI_Initialize(void)
{
 ...
 // MCP2515_WriteByte(MCP2515_CNF3, 0x83);
   ...
   mcp2515_configRate(  CAN_250KBPS) ;
...
#ifdef debugcan
    printf("\r\n_Transmit0  id: %x lg: %x  %x %x %x %x  %x %x %x %x  \r\n",tempCanMsg->frame.id , tempCanMsg->frame.dlc,
			 tempCanMsg->frame.data0	,
			 tempCanMsg->frame.data1	,
			 tempCanMsg->frame.data2	,
			 tempCanMsg->frame.data3    ,
   			 tempCanMsg->frame.data4	,
   			 tempCanMsg->frame.data5	,
   			 tempCanMsg->frame.data6	,
   			 tempCanMsg->frame.data7
...
#ifdef debugcan
	 printf("\r\nTransmit1  id: %x lg: %x  %x %x %x %x %x %x %x %x\r\n",tempCanMsg->frame.id , tempCanMsg->frame.dlc,
			 tempCanMsg->frame.data0	,
			 tempCanMsg->frame.data1	,
			 tempCanMsg->frame.data2	,
			 tempCanMsg->frame.data3    ,
...
#ifdef debugcan
    printf("\r\nCANSPI_Transmit2  id: %x lg: %x  %x %x %x %x \r\n",tempCanMsg->frame.id , tempCanMsg->frame.dlc,
   			 tempCanMsg->frame.data0	,
   			 tempCanMsg->frame.data1	,
   			 tempCanMsg->frame.data2	,
   			 tempCanMsg->frame.data3   ,
...
//#endif
 // taskEXIT_CRITICAL();
  if (returnValue==0)
  {  printf (" \r\n ** buffer tx buff :  CANSPI_isTxErrorPassive=%u  CANSPI_isBussOff=%u",
  CANSPI_isTxErrorPassive() ,  CANSPI_isBussOff());
...
//	  taskENTER_CRITICAL();
  ...
  if (rxStatus.rxBuffer != 0)
  {
    ...
    printf("\r\n CANSPI_Receive message  id: %x lg: %x  %x %x %x %x %x %x %x %x \r\n",tempCanMsg->frame.id , tempCanMsg->frame.dlc,
   			 tempCanMsg->frame.data0	,
   			 tempCanMsg->frame.data1	,
   			 tempCanMsg->frame.data2	,
   			 tempCanMsg->frame.data3,
